...
extern struct tm SNTP_time;

static void RTC_Set_Date_SNTP(void);
...
{
	osThreadFlagsSet (TID_RTC_LED, 0x01);
}
...
//	HAL_RTCEx_SetWakeUpTimer_IT(&hrtc, WakeUpCounter, RTC_WAKEUPCLOCK_RTCCLK_DIV16);
  ...
  HAL_NVIC_SetPriority(RTC_Alarm_IRQn, 100, 0);
...
/**
  ...
  * @retval None
  */
void RTC_CalendarConfig(bool SNTP)
{
...
static void RTC_Set_Date_SNTP(void)
{
  ...
  sdatestructure.Year = SNTP_time.tm_year;
  sdatestructure.Month = SNTP_time.tm_mon + 1;
  sdatestructure.Date = SNTP_time.tm_mday;
...
static void RTC_Set_Time_SNTP(void)
{
  ...
  stimestructure.Hours = SNTP_time.tm_hour + 1;
  stimestructure.Minutes = SNTP_time.tm_min;
  stimestructure.Seconds = SNTP_time.tm_sec;
