...
extern uint8_t RTC_Time[60];
extern uint8_t RTC_Date[60];
  ...
  switch (env[0]) {
    ...
    case 'g':
      ...
      break;
		 ...
		 case 'h':
      // Fecha Input from 'fecha.cgi'
      switch (env[2]) {
        case '1':
          len = (uint32_t)sprintf (buf,&env[4], RTC_Date);
          break;
        case '2':
          len = (uint32_t)sprintf (buf,&env[4], RTC_Time);
          break;
      }
      break;
		
		case 'i':
      // RTC Fecha Input from 'fecha.cgx'
      len = (uint32_t)sprintf (buf, &env[1], RTC_Date);
			break;
		
		case 'j':
      // RTC Hora Input from 'fecha.cgx'
      len = (uint32_t)sprintf (buf, &env[1], RTC_Time);
			break;
